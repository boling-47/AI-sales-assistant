"""
AI销售分析助手 - 分模块分析版
功能：上传销售Excel -> 销售概况 -> 产品分析 -> 趋势预测 -> 客户/地域/销售行为分Tab分析
"""
...
if 'customer_report' not in st.session_state:
    st.session_state.customer_report = None
if 'geo_report' not in st.session_state:
    st.session_state.geo_report = None
...
def get_customer_report(data):
    prompt = f"""你是一名销售分析师。根据以下客户分析数据生成报告：

客户总数：{data.get("客户总数", "无")}
活动期间新增客户：{data.get("活动新增客户", "无")}
客户保持率：{data.get("保持率", "无")}
客户流失率：{data.get("流失率", "无")}
高价值客户偏好产品：{data.get("高价值偏好", "无")}
忠诚客户偏好产品：{data.get("忠诚偏好", "无")}
价格敏感型客户占比：{data.get("价格敏感占比", "无")}
品质导向型客户占比：{data.get("品质导向占比", "无")}
...
def get_geo_report(data):
    prompt = f"""你是一名销售分析师。根据以下地域分析数据生成报告：

覆盖地区数：{data.get("地区数", "无")}
客户最多地区：{data.get("客户最多地区", "无")}
最稳定地区：{data.get("最稳定地区", "无")}
价格敏感地区：{data.get("价格敏感地区", "无")}
品质导向地区：{data.get("品质导向地区", "无")}
各区域产品偏好：{data.get("区域偏好", "无")}
...
def get_behavior_report(data):
    prompt = f"""你是一名销售分析师。根据以下销售行为分析数据生成报告：

活动拉动效果：{data.get("活动拉动", "无")}
节假日对销量影响：{data.get("节假日影响", "无")}
季节性趋势：{data.get("季节性", "无")}
价格变动对销量影响：{data.get("价格影响", "无")}
...
# ============================================================
st.title("📊 AI销售分析助手")
st.write("上传Excel文件，自动分析销售数据、客户画像、地域分布、销售行为，分模块生成报告")
...
if has_customer_data:
    ...
    with col5:
        st.metric("人均消费", f"¥{int(total_sales / df['用户ID'].nunique()):,}")
...
# 3. 销售趋势
# ==============================================
trend = df.groupby('日期')["销量"].sum().reset_index()

st.write("### 📈 销售趋势")
fig_line = px.line(trend, x='日期', y="销量", title="销售数量趋势")
...
# ==============================================
st.write("---")
tab1, tab2, tab3 = st.tabs(["👥 客户分析", "🗺️ 地域分析", "📣 销售行为分析"])
...
with tab1:
    if not has_customer_data:
        st.info("当前数据没有「用户ID」列，无法生成客户分析。请在Excel中添加「用户ID」列。")
    else:
        ...
        high_value_pct = round(int(high_value_customers['消费总额'].sum()) / total_sales * 100, 1)
        ...
        # AI报告按钮
        st.write("---")
        if st.button("🤖 生成客户分析AI报告", key="btn_customer"):
            with st.spinner("AI正在生成客户分析报告..."):
                st.session_state.customer_report = get_customer_report(customer_data)
        if st.session_state.customer_report:
            with st.container(border=True):
                st.markdown(st.session_state.customer_report)
...
with tab2:
    if not has_geo_data:
        st.info("当前数据没有「配送地址」列，无法生成地域分析。请在Excel中添加「配送地址」列。")
    else:
        ...
        fig_heat.update_layout(xaxis_title="产品", yaxis_title="省份", font=dict(color='black', size=12))
        ...
        # AI报告按钮
        st.write("---")
        if st.button("🤖 生成地域分析AI报告", key="btn_geo"):
            with st.spinner("AI正在生成地域分析报告..."):
                st.session_state.geo_report = get_geo_report(geo_data)
        if st.session_state.geo_report:
            with st.container(border=True):
...
with tab3:
    ...
    # AI报告按钮
    st.write("---")
    if st.button("🤖 生成销售行为分析AI报告", key="btn_behavior"):
        with st.spinner("AI正在生成销售行为分析报告..."):
            st.session_state.behavior_report = get_behavior_report(behavior_data)
    if st.session_state.behavior_report:
        with st.container(border=True):
            st.markdown(st.session_state.behavior_report)
...
def create_word_report():
    ...
    all_images = [
        "sales_chart.png", "money_chart.png",
        "line_chart.png", "forecast_chart.png",
    ...
    reports = [
        ("👥 客户分析报告", st.session_state.get('customer_report')),
        ("🗺️ 地域分析报告", st.session_state.get('geo_report')),
        ("📣 销售行为分析报告", st.session_state.get('behavior_report')),
    ]
...
has_any_report = any([st.session_state.get('customer_report'), st.session_state.get('geo_report'), st.session_state.get('behavior_report')])

if has_any_report:
    word_file = create_word_report()
    st.download_button(
        label="📄 导出Word报告（包含已生成的AI报告）",
        data=word_file,
        file_name="销售分析报告.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    )
else:
    st.caption("💡 先在上方各Tab中点击「生成AI报告」按钮，再导出Word。")
